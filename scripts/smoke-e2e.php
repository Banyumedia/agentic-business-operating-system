...
if (Company::where('owner_user_id', $user->id)->count() !== 0) {
    note('Registrasi membuat company (harusnya tidak — D-26)');
} else {
    ok('Registrasi tidak membuat company (benar)');
}
...
ModuleSetting::create(['company_id' => $company->id, 'module_name' => 'features', 'settings_json' => []]);
...
foreach ($modules as $path => $label) {
    ...
    if ($code === 200) {
        ...
        ok("$label ($path) -> 302 (redirect, kemungkinan auth/module gating)");
    } else {
        note("$label ($path) -> $code (tak terduga)");
    }
}
...
$analyzer = app(BusinessHealthAnalyzer::class);
try {
    $health = $analyzer->analyze();
    if (! empty($health['insufficient_data'])) {
        note('Panel kesehatan: insufficient_data padahal ada 1 transaksi (mungkin butuh lebih banyak data)');
    } else {
...
if (! $plan) {
    ...
    try {
        ...
        if ($code !== 200) {
            note("Halaman instruksi bayar -> $code (harusnya 200)");
        } else {
        ...
        // Konfirmasi admin
        $confirm = app(InvoiceConfirmationService::class);
        $confirm->confirmPayment($inv);
        $active = $company->memberships()->where('status', 'active')->latest('id')->first();
        if ($active && $active->plan_id === $plan->id) {
            ok("Setelah konfirmasi: membership AKTIF paket {$plan->name}");
        } else {
            note('Setelah konfirmasi: membership tidak aktif / plan salah');
        }
        // Idempoten: konfirmasi kedua kali tidak mengkredit dua kali
        try {
            $confirm->confirmPayment($inv->fresh());
            ok('Idempoten: konfirmasi ulang aman (tidak dobel)');
        } catch (Throwable $e) {
            ok("Idempoten: konfirmasi ulang ditolak ({$e->getMessage()})");
        }
    } catch (Throwable $e) {
...
if ($leak) {
    note('KEBOCORAN TENANT: data company1 terbaca di company2!');
} else {
    ok('Tenant isolation: data company1 TIDAK terbaca di company2');
}
// Coba akses eksplisit company1 dari konteks company2
try {
    $repo->for((string) $company->id, 'contacts')->all();
    note('KEBOCORAN: akses lintas company tidak ditolak');
} catch (Throwable $e) {
    ok('Akses lintas company ditolak (benar)');
}

echo "\n=== RINGKASAN TEMUAN ===\n";
if (empty($findings)) {
    echo "TIDAK ADA TEMUAN — semua skenario bersih.\n";
} else {
    echo count($findings)." temuan:\n";

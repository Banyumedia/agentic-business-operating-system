...
| D-43 | **Palet default: A (Slate + Emerald)** (Bos, 2026-09-16) | Nilai di `UX_UI_SPEC.md` §7.3a kolom A menjadi §7.3/`app.css`. B (Zinc+Amber), C (Navy+Sky), D (Stone+Terracotta) = tema pilihan owner via `[data-theme]`; tema ke-5 = mode terang dari A. Accent picker per tenant tetap di atas tema. Success dipetakan emerald-300 agar berbeda dari aksen. |
...
| D-45 | **Konfirmasi aksi berisiko: ketik ulang, bukan tekan-tahan** | Aksi destruktif/finansial di web tetap memakai *type-to-confirm* dengan input dinormalisasi `strtoupper(trim())`. **Tekan-dan-tahan ditolak** untuk aksi finansial: tidak membuktikan niat, rawan tidak sengaja, dan tidak meninggalkan jejak tekstual untuk audit. Aksi destruktif **non-finansial** boleh 2 langkah biasa (konfirmasi + Batal). Jalur `YA <kode-tiket>` (D-27) tetap khusus persetujuan via WhatsApp, bukan pengganti modal web. Teks modal wajib menyebut akibat konkret (nama & nominal), bukan "Anda yakin?". |

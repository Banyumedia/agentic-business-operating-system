...
|---|---|---|---|---|---|---|
| T-07 | Design token `--erp-*` + tema + Settings shell | T-03 | D-36, D-43 (palet A locked) | `resources/css/app.css`, `app/Services/ThemeRegistry.php`, `app/Livewire/Settings.php` | (a) 36 token `@theme` sesuai UX §7; (b) `ThemeContrastTest` 11 pasangan >= 4.5:1 per tema; (c) `/app/settings` `role=tablist` dari array registry; (d) toggle `auto/light/dark` di `localStorage` + `<html data-theme>`; (e) Pint + build hijau. | `READY` |

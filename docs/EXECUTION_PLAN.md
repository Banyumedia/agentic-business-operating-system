...
| T-20 | Scout + Universal Search nyata | T-13, T-12, T-08b, T-08c | D-35 (`database`, locked) | `HUMAN:APPROVAL` (composer) | `composer require laravel/scout`, `Searchable` di `Contact`, `Deal`, `Project`, `Invoice`, `Item`; `app/Livewire/CommandPalette.php` | Hasil dari DB, scoped `company_id`, label via `term()`; dummy & `href="#"` dihapus; test: A tidak melihat B; hasil menampilkan `Pasien` untuk preset `pharmacy`. | `BLOCKED` |

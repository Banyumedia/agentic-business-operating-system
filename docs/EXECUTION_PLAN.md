...
| T-F5 | `FeatureResolver` + `TerminologyResolver` + `term()` + `@term` | T-F4, T-F3 | — | `app/Services/FeatureResolver.php`, `TerminologyResolver.php`, `app/Support/helpers.php` | Resolusi override company (`storage/app/json/{c}/settings.json`) -> preset -> default global §3. Test: `klinik`->`term('contact')='Pasien'`, `bengkel`->`'Pelanggan'`; key asing -> exception di dev. **Menggantikan T-08b/T-08c Fase 3b**; Fase 3 hanya mengganti sumber. | `READY` |

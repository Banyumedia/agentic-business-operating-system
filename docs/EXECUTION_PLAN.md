...
| T-13d | `items`, `item_batches`, `stock_movements`, `bom_lines` (§7) + `StockService` | T-00a | — | migration ×4 + model + service | Efek `stock.reserve/deduct`; FEFO: deduct mengambil batch `expires_on` terdekat (test dengan 3 batch); BOM: produce 1 produk → consume komponen sesuai `bom_lines`; test A/B. | `READY` |

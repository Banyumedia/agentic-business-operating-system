...
| T-24b | Preset gelombang 2: tambah `kursus`, `kos_coworking`; validasi ulang preset kanonik `bengkel` dan `laundry` | T-24 | — | 2 JSON baru + `PresetCompositionTest` untuk empat preset: kapabilitas, `term()`, workflow default, widget dashboard sesuai §7.3/§7.6/§7.7/§7.9. Workflow `laundry` yang sudah dibuat T-F15 tetap memicu `notify.owner_wa`. **Diff kode = 0.** | `DONE 5a0ba6d` |
